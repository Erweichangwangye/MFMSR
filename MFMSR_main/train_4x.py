...
import math
# ---load model arc---
from model_archs.mfmsr_mamba import MFMSR as net
...
parser.add_argument('--val_dir', type=str, default='datasets/val')  # val while training
...
parser.add_argument('--pretrained_sr', default='saved_models/mfmsr/15-04-41/mfmsr_epoch_10.pth', help='sr pretrained base model')
parser.add_argument('--pretrained', type=bool, default=False)
parser.add_argument('--save_folder', default='saved_models/mfmsr/', help='Location to save checkpoint models')
parser.add_argument('--log_folder', default='/mfmsr/', help='Location to save checkpoint models')
...
def bestCheckpoint():
    model_out_path = opt.save_folder + opt.model_type + "best.pth"
    torch.save(model.state_dict(), model_out_path)
    print("Checkpoint saved to {}".format(model_out_path))
rewrite_print = print
def print_log(*arg):
...
for epoch in range(opt.start_epoch, opt.nEpochs + 1):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        optimizer.step()
        writer.add_scalar('loss',loss.item(),iteration*epoch)
        t1 = time.time()
        print("===> Epoch[{}]({}/{}): Loss: {:.4f} || Timer: {:.4f} sec || resNeeds: {:.4f} min || TotalEpoch: {}.".format(epoch, iteration, len(training_data_loader),
                                                                                 loss.item(), (t1 - t0), ((t1 - t0)*(len(training_data_loader)*opt.nEpochs-(epoch-1)*len(training_data_loader)-iteration)/60),opt.nEpochs))
    ...
    for batch in val_data_loader:
        ...
        with torch.no_grad():
            ...
            t1 = time.time()
        save_path = opt.save_folder + "/val/"+str(epoch)
        os.makedirs(save_path, exist_ok=True)
        torchvision.utils.save_image(prediction.squeeze(0),os.path.join(save_path,str(count)+".png"))
    ...
    if epoch % (opt.snapshots) == 0 or epoch==1:
        checkpoint(epoch)
